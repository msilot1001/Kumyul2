...
import IParagraph from '../interfaces/IParagraph.js';
import {
  colorPredicate,
...
export default class PageBuilder {
  ...
  /**
   *
   * @param url Allowed Protocols : (http:) , (https:) , (attachment:)
   * @returns
   */
  ...
  /**
   *
   * @param timestamp ISO8601 timestamp (default: Date.now())
   * @returns
   */
  ...
  /**
   *
   * @param url Allowed Protocols : (http:) , (https:)
   * @returns
   */
  ...
  public spliceActionRows(
    ...
    s.number
      ...
      .setValidationEnabled(true)
      .parse(
        (this.data.actionRows?.length ?? 0) + actionRows.length - deleteCount,
      );

    // if actionrow exists
    if (this.data.actionRows)
      this.data.actionRows.splice(index, deleteCount, ...actionRows);
    // or not
    else this.data.actionRows = actionRows;

    return this;
  }

  public addButtonRow(...buttons: RestOrArray<ButtonBuilder>): this {
    // eslint-disable-next-line no-param-reassign
    buttons = normalizeArray(buttons);

    // Ensure buttons won't exceed 5 buttons limit
    buttonQuantityPredicate.parse(buttons.length);

    const actionRow = new ActionRowBuilder();

    // Ensure adding these actionrows won't exceed the 5 rows limit
    validateFieldLength(1, this.data.paragraphs);

    if (this.data.actionRows) this.data.actionRows.push(actionRow);
    else this.data.actionRows = [actionRow];
